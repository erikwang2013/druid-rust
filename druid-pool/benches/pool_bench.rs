...
impl Connection for BenchConn {
    async fn execute(&self, _: &str) -> Result<u64, druid_core::DruidError> {
        Ok(1)
    }
    async fn query(&self, _: &str) -> Result<Vec<Vec<String>>, druid_core::DruidError> {
        Ok(vec![])
    }
    async fn close(&self) -> Result<(), druid_core::DruidError> {
        Ok(())
    }
    async fn ping(&self) -> Result<(), druid_core::DruidError> {
        Ok(())
    }
...
impl Driver for BenchDriver {
    ...
    async fn validate(&self, _: &BenchConn) -> Result<(), druid_core::DruidError> {
        Ok(())
    }
}
