...
mod mock {
    ...
    #[derive(Debug, Clone)]
    pub struct MockConn { pub id: u64, pub closed: Arc<std::sync::Mutex<bool>> }
    ...
    #[derive(Debug)]
    pub struct MockDriver { pub count: AtomicU64 }
    ...
    impl Driver for MockDriver {
        ...
        async fn connect(&self, _: &str, _: &str, _: &str) -> Result<MockConn, DruidError> {
            ...
            Ok(MockConn { id, closed: Arc::new(std::sync::Mutex::new(false)) })
        }
        fn name(&self) -> &'static str { "Mock" }
        async fn validate(&self, _: &MockConn) -> Result<(), DruidError> { Ok(()) }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ds = DruidDataSource::new(mock::MockDriver { count: Default::default() }, config);
    ds.init().await?;
    println!("Pool: active={}, idle={}", ds.active_count(), ds.idle_count());

    let guard = ds.get_connection().await?;
    println!("Borrowed #{} (active={})", guard.connection_id(), ds.active_count());
    drop(guard);
