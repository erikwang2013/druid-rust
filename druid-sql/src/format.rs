...
fn format_select(stmt: &SelectStatement, _indent: usize) -> String {
    ...
    s.push_str("SELECT ");
    if stmt.distinct { s.push_str("DISTINCT "); }

    let cols: Vec<String> = stmt.columns.iter().map(|item| match item {
    ...
    if !stmt.order_by.is_empty() {
        let ob: Vec<String> = stmt.order_by.iter().map(|o| {
            let mut es = format_expr(&o.expr);
            if !o.asc { es.push_str(" DESC"); }
            es
        }).collect();
...
fn format_insert(stmt: &InsertStatement) -> String {
    ...
    s.push_str(" VALUES ");
    let rows: Vec<String> = stmt.values.iter().map(|row| {
...
fn format_update(stmt: &UpdateStatement) -> String {
    ...
    let sets: Vec<String> = stmt.sets.iter().map(|(col, val)| {
        format!("{} = {}", col, format_expr(val))
    }).collect();
...
fn format_create_table(stmt: &CreateTableStatement) -> String {
    ...
    if stmt.if_not_exists { s.push_str("IF NOT EXISTS "); }
...
fn format_drop_table(stmt: &DropTableStatement) -> String {
    let mut s = format!("DROP TABLE ");
    if stmt.if_exists { s.push_str("IF EXISTS "); }
...
fn format_table_ref(tr: &TableReference) -> String {
    match tr {
        TableReference::Table { name, alias, schema } => {
            let mut s = if let Some(sch) = schema { format!("{}.{}", sch, name) }
                        else { name.clone() };
            if let Some(a) = alias { write!(s, " {}", a).unwrap(); }
            s
        }
...
pub fn format_expr(expr: &SQLExpr) -> String {
    match expr {
        ...
        SQLExpr::Function { name, args, distinct } => {
        ...
        SQLExpr::Between { expr, low, high, not } => {
            let n = if *not { "NOT " } else { "" };
            format!("{} {}BETWEEN {} AND {}", format_expr(expr), n, format_expr(low), format_expr(high))
        }
        ...
        SQLExpr::Case { expr, whens, else_expr } => {
            let mut s = "CASE".to_string();
            if let Some(e) = expr { write!(s, " {}", format_expr(e)).unwrap(); }
            for (cond, result) in whens {
                write!(s, " WHEN {} THEN {}", format_expr(cond), format_expr(result)).unwrap();
            }
            if let Some(e) = else_expr { write!(s, " ELSE {}", format_expr(e)).unwrap(); }
            s.push_str(" END");
            s
        }
